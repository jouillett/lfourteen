  ...
  return (
    <div className="max-w-3xl mx-auto bg-surface-container-lowest border border-outline-variant rounded-xl p-8 shadow-sm">
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="mb-6">
          <label className="block text-sm font-bold text-on-surface-variant mb-1">Address (배송지 주소)</label>
          <input type="text" readOnly value={order.receiver_address || '-'} className="w-full bg-surface-container border border-outline-variant rounded-md px-4 py-2 text-on-surface focus:outline-none cursor-not-allowed" />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-bold text-on-surface-variant mb-1">Total Price</label>
            <input type="text" readOnly value={`${order.wholesale_price ? Number(order.wholesale_price).toLocaleString() : '0'}원`} className="w-full bg-surface-container border border-outline-variant rounded-md px-4 py-2 text-on-surface focus:outline-none cursor-not-allowed" />
          </div>
          <div>
